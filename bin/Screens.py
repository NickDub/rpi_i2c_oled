...
TITLE_BASE = 2 * (TEXT_SPACE + FONT_WIDTH)
TITLE_TEXT_WITH_ICON = TITLE_BASE + TEXT_SPACE + FONT_WIDTH
LINE_BASE = 0
LINE_TEXT_WITH_ICON = LINE_BASE + TEXT_SPACE + FONT_WIDTH

LINE_1 = 0
LINE_2 = LINE_1 + FONT_HEIGHT  # 16
LINE_3 = LINE_2 + FONT_HEIGHT  # 32
LINE_4 = LINE_3 + FONT_HEIGHT  # 48

# Icon constants
ICON_BARSPROGRESS = chr(int("0xf828", 0))
ICON_ETHERNET = chr(int("0xf796", 0))
ICON_GLOBE = chr(int("0xf0ac", 0))
ICON_HARDDRIVE = chr(int("0xf0a0", 0))
ICON_MEMORY = chr(int("0xf538", 0))
ICON_MICROCHIP = chr(int("0xf2db", 0))
ICON_NETWORKWIRED = chr(int("0xf6ff", 0))
...
class NetworkScreen(BaseScreen):
    def render(self):
        ...
        self.display.draw.text(
            (TITLE_BASE, LINE_1),
            ICON_NETWORKWIRED,
            font=self.font(16, is_bold=False, is_icon=True),
            fill=255,
        )
        self.display.draw.text(
            (LINE_BASE, LINE_2),
        ...
        # Draw Text
        self.display.draw.text(
            (TITLE_TEXT_WITH_ICON, LINE_1), "NETWORK", font=self.font(16), fill=255
        )
        self.display.draw.text(
            (LINE_TEXT_WITH_ICON, LINE_2), hostname, font=self.font(16), fill=255
        )
        self.display.draw.text(
            (LINE_TEXT_WITH_ICON, LINE_3), ipv4, font=self.font(16), fill=255
        )
...
class StorageScreen(BaseScreen):
    def render(self):
        drive = "/"
        storage = Utils.shell_cmd(
            "df "
            + drive
            + ' | awk \'$NF=="/"{printf "%.1f,%.1f,%.1f", $3/(1024*1024), $2/(1024*1024), 100*($2-$3)/$2 }\''
        ).split(",")
        used = float(storage[0])
        total = float(storage[1])
        free_percent = float(storage[2])
        ...
        # Draw Text
        self.display.draw.text(
            (TITLE_TEXT_WITH_ICON, LINE_1), "STORAGE", font=self.font(16), fill=255
        )
        self.display.draw.text(
            (LINE_TEXT_WITH_ICON, LINE_2),
            "Used: " + str(used) + "/" + str(total) + "G",
            font=self.font(16),
            fill=255,
        )
        self.display.draw.text(
            (LINE_TEXT_WITH_ICON, LINE_3),
            "Free: " + str(free_percent) + "%",
            font=self.font(16),
            fill=255,
        )
...
class MemoryScreen(BaseScreen):
    def render(self):
        mem = Utils.shell_cmd(
            "free -m | awk 'NR==2{printf \"%.1f,%.1f,%.1f\", $3/1024, $2/1024, 100*$4/$2 }'"
        ).split(",")
        used = float(mem[0])
        total = float(mem[1])
        free_percent = float(mem[2])
        ...
        # Draw Text
        self.display.draw.text(
            (TITLE_TEXT_WITH_ICON, LINE_1), "MEMORY", font=self.font(16), fill=255
        )
        self.display.draw.text(
            (LINE_TEXT_WITH_ICON, LINE_2),
            "Used: " + str(used) + "/" + str(total) + "G",
...
class CpuScreen(BaseScreen):
    ...
    def get_temp(self):
        temp = (
            float(Utils.shell_cmd("cat /sys/class/thermal/thermal_zone0/temp"))
            / 1000.00
        )
    ...
    def render(self):
        ...
        # Draw Text
        self.display.draw.text(
            (TITLE_TEXT_WITH_ICON, LINE_1), "CPU", font=self.font(16), fill=255
        )
        self.display.draw.text(
            (LINE_TEXT_WITH_ICON, LINE_2), str(temp), font=self.font(16), fill=255
        )
        self.display.draw.text(
            (83, LINE_2), str(cpu) + "LA", font=self.font(16), fill=255
        )
        self.display.draw.text(
            (LINE_TEXT_WITH_ICON, LINE_3), uptime, font=self.font(16), fill=255
        )
...
class StatsScreen(BaseScreen):
    ...
    def render(self):
        ...
        storage = Utils.shell_cmd(
            'df -h | awk \'$NF=="/"{printf "Disk: %d/%dG %s", $3,$2,$5}\''
        )

        if self.display.icon_stats == "text":
            self.display.draw.text(
                (LINE_BASE, LINE_1), "IP: " + ipv4, font=self.font(16), fill=255
            )
            self.display.draw.text(
                (LINE_BASE, LINE_2),
                "CPU: " + str(cpu) + "LA",
            ...
            self.display.draw.text(
                (LINE_BASE, LINE_3), mem, font=self.font(16), fill=255
            )
            self.display.draw.text(
                (LINE_BASE, LINE_4), storage, font=self.font(16), fill=255
            )
        else:
            storage = Utils.shell_cmd(
                'df -h | awk \'$NF=="/"{printf "%d/%dG", $3,$2}\''
            )
            mem = Utils.shell_cmd(
            ...
            # Draw Text
            self.display.draw.text(
                (LINE_TEXT_WITH_ICON, LINE_1), str(temp), font=self.font(16), fill=255
            )
            self.display.draw.text((87, LINE_1), str(mem), font=self.font(16), fill=255)
            self.display.draw.text(
                (LINE_TEXT_WITH_ICON, LINE_2),
                str(storage),
                font=self.font(16),
            ...
            self.display.draw.text(
                (87, LINE_2), str(cpu) + "LA", font=self.font(16), fill=255
            )
            self.display.draw.text(
                (LINE_TEXT_WITH_ICON, LINE_3), ipv4, font=self.font(16), fill=255
            )
