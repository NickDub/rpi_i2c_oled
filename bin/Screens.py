...
class StorageScreen(BaseScreen):
    def render(self):
        drive = "/"
        storage = Utils.shell_cmd("df " + drive + ' | awk \'$NF=="/"{printf "%.1f,%.1f,%.1f", $3/(1024*1024), $2/(1024*1024), 100*($2-$3)/$2 }\'').split(",")
        ...
        self.display.draw.text((LINE_TEXT_WITH_ICON, LINE_2), "Used: " + str(used) + "/" + str(total) + "G",font=self.font(16), fill=255)
        self.display.draw.text((LINE_TEXT_WITH_ICON, LINE_3), "Free: " + str(free_percent) + "%",font=self.font(16), fill=255)
...
class MemoryScreen(BaseScreen):
    def render(self):
        mem = Utils.shell_cmd("free -m | awk 'NR==2{printf \"%.1f,%.1f,%.1f\", $3/1024, $2/1024, 100*$4/$2 }'"
        ).split(",")
        used = float(mem[0])
        total = float(mem[1])
...
class CpuScreen(BaseScreen):
    ...
    def render(self):
        ...
        self.display.draw.text((LINE_BASE, LINE_3), ICON_STOPWATCH, font=self.font(16, is_bold=False, is_icon=True), fill=255)
        # Draw Text
        self.display.draw.text((TITLE_TEXT_WITH_ICON, LINE_1), "CPU", font=self.font(16), fill=255)
        self.display.draw.text((LINE_TEXT_WITH_ICON, LINE_2), str(temp), font=self.font(16), fill=255)
        self.display.draw.text((83, LINE_2), str(cpu) + "LA", font=self.font(16), fill=255)
        self.display.draw.text((LINE_TEXT_WITH_ICON, LINE_3), uptime, font=self.font(16), fill=255)
...
class StatsScreen(BaseScreen):
    ...
    def render(self):
        ...
        if self.display.icon_stats == "text":
            self.display.draw.text((LINE_BASE, LINE_1), "IP: " + ipv4, font=self.font(16), fill=255)
            ...
            self.display.draw.text((LINE_BASE, LINE_3), mem, font=self.font(16), fill=255)
            self.display.draw.text((LINE_BASE, LINE_4), storage, font=self.font(16), fill=255)
        else:
            storage = Utils.shell_cmd('df -h | awk \'$NF=="/"{printf "%d/%dG", $3,$2}\'')
            ...
            self.display.draw.text((LINE_BASE, LINE_3), ICON_WIFI, font=self.font(16, is_bold=False, is_icon=True), fill=255)
            # Draw Text
            self.display.draw.text((LINE_TEXT_WITH_ICON, LINE_1), str(temp), font=self.font(16), fill=255)
            self.display.draw.text((87, LINE_1), str(mem), font=self.font(16), fill=255)
            self.display.draw.text((LINE_TEXT_WITH_ICON, LINE_2), str(storage), font=self.font(16), fill=255)
